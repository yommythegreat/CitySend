...
// ── ChatPanel (pure display — no own subscription) ────────────────────────────
// All message state lives in DeliveryScreen to prevent duplicate subscriptions.
...
function ChatPanel({
  order, myId, messages, fetchError, sending, inputText, callNotice,
  onSend, onInputChange, onRetry, onDismissCallNotice, onClose,
}: ChatPanelProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>
            {order.customerId ? order.dropoff.name : order.pickup.name}
          </div>
      ...
      {callNotice && (
        <div style={{
          ...
          <span style={{ flex: 1, fontSize: 13, color: '#92400e' }}>
            <strong>Calling is not available yet.</strong> Please message the customer instead.
          </span>
          <button
      ...
      <div style={{
        ...
        {fetchError ? (
          <div style={{ textAlign: 'center', padding: '40px 16px' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--d-muted)', marginBottom: 12, fontFamily: 'monospace' }}>
              {fetchError}
            </div>
            <button
              onClick={onRetry}
              style={{
                padding: '8px 18px', border: '1.5px solid var(--d-border)',
                borderRadius: 10, background: '#fff', fontSize: 13, cursor: 'pointer',
              }}
            >Retry</button>
          </div>
        ) : messages.length === 0 ? (
              ...
              <div key={m.id} style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    {fmtTime(m.createdAt)}
                    {isMine && (
                      <span style={{ color: m.isRead ? '#4ade80' : 'inherit', fontSize: 11 }}>
                        {m.isRead ? '✓✓' : '✓'}
                      </span>
                    )}
                  </div>
      ...
      {!isTerminal && !fetchError && (
        <div style={{
          display: 'flex', gap: 8, padding: '6px 12px', overflowX: 'auto',
          scrollbarWidth: 'none', flexShrink: 0, background: 'var(--d-surface)',
        }}>
          {QUICK_REPLIES.map(reply => (
            <button
              ...
              style={{
                ...
                color: 'var(--d-accent)', fontSize: 12, fontWeight: 600,
      ...
      {isTerminal ? (
        <div style={{
          ...
          background: '#fff', borderTop: '1px solid var(--d-border)',
        ...
        <div style={{
          ...
          <textarea
            ...
            rows={1}
            style={{
              flex: 1, resize: 'none', border: '1.5px solid var(--d-border)',
          ...
          <button
            ...
            style={{
              ...
              background: inputText.trim() && !sending ? 'var(--d-accent)' : 'var(--d-border)',
              color: inputText.trim() && !sending ? '#fff' : 'var(--d-muted)',
              fontSize: 16, cursor: inputText.trim() && !sending ? 'pointer' : 'default',
...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
      ...
      <span style={{ fontSize: 13, color: 'var(--d-ink)', fontWeight: 500, textAlign: 'right', marginLeft: 12 }}>
        {value}
      </span>
    </div>
  )
...
function ContactRow({ onCall, onMessage, unreadCount }: ContactRowProps) {
  return (
    <div className="d-contact-row">
      {/* Call — disabled, shows notice */}
      <button className="d-contact-btn" onClick={onCall}>
      ...
      {/* Message — opens chat */}
      <button
        className="d-contact-btn"
        onClick={onMessage}
        style={{
          position: 'relative',
          background: unreadCount > 0 ? 'rgba(201,74,27,0.06)' : undefined,
          borderColor: unreadCount > 0 ? 'var(--d-accent)' : undefined,
          color:       unreadCount > 0 ? 'var(--d-accent)'  : undefined,
        }}
      >
        <span style={{ fontSize: 15 }}>💬</span>
        <span>Message</span>
...
export function DeliveryScreen({ orderId, onBack, onComplete, initialChatOpen = false }: Props) {
  ...
  // ── Message state (single source — no dual subscription) ──────────────────
  ...
  // ── Load + subscribe (ONCE per orderId — never tied to chatOpen) ──────────
  const loadMessages = useCallback(async () => {
    if (!orderId) return
    console.log('[DeliveryScreen] loadMessages orderId=', orderId)
    try {
      const msgs = await getMessages(orderId)
      setMessages(msgs)
      setFetchError(null)
    } catch (err: any) {
      console.error('[DeliveryScreen] loadMessages ERROR', err)
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToMessages(orderId, (msgs) => {
      console.log('[DeliveryScreen] realtime update', msgs.length, 'messages')
      setMessages(msgs)
    })
    return unsub
  }, [orderId])  // ← chatOpen is NOT a dependency; single stable subscription

  // Mark messages read when chat is open
  useEffect(() => {
    if (chatOpen && myId && messages.length > 0) {
      markMessagesRead(orderId, myId).catch(err =>
        console.warn('[DeliveryScreen] markMessagesRead error', err),
      )
    }
  }, [chatOpen, orderId, myId, messages.length])

  // Show toast for incoming messages when chat is closed
  ...
  useEffect(() => {
    ...
    if (count > prevUnreadRef.current) {
      ...
      if (latest) {
        setToast(`💬 Customer: "${latest.messageText.slice(0, 40)}${latest.messageText.length > 40 ? '…' : ''}"`)
  ...
  // ── Safeguard: order not found ────────────────────────────────────────────
  if (!order) {
    const hasAuth = !!state.auth
    return (
      <div style={{
        position: 'fixed', inset: 0, background: 'var(--d-bg)',
        ...
        {hasAuth ? (
          <>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--d-ink)', marginBottom: 6 }}>
              Loading order…
            </div>
            <div style={{ fontSize: 13, color: 'var(--d-muted)', marginBottom: 24 }}>
              {orderId}
            </div>
          </>
        ) : (
          <>
            ...
            <div style={{ fontSize: 13, color: 'var(--d-muted)', marginBottom: 24 }}>
              This order may have been reassigned or does not exist.
            </div>
          </>
        )}
        <button
          onClick={onBack}
          style={{
            padding: '10px 24px', background: 'var(--d-accent)', border: 'none',
  ...
  const handleSend = useCallback(async (text?: string) => {
    ...
    setSending(true)
    if (!text) setInputText('')   // clear field only for typed messages, not quick replies
    console.log('[DeliveryScreen] sending', { orderId: order.id, senderId: myId, receiverId: customerId })
    try {
      await sendMessage({
        orderId: order.id, senderId: myId, senderRole: 'driver',
        receiverId: customerId, receiverRole: 'customer', messageText: msg,
      })
      // Re-fetch after send — ensures message appears even if realtime is slow
      await loadMessages()
    } catch (err: any) {
      console.error('[DeliveryScreen] send ERROR', err)
      ...
      return (
        <>
          ...
          <div style={{ fontSize: 14, color: 'var(--d-muted)', marginBottom: 16 }}>
            {order.pickup.address.split(',').slice(1).join(',')}
            {order.pickup.unit ? ` · Unit ${order.pickup.unit}` : ''}
          </div>

          {/* Sender info + Call/Message */}
          <div style={{ background: '#f8f9fb', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <div style={{
                width: 36, height: 36, borderRadius: '50%', background: 'var(--d-accent)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
            ...
            <div style={{ flex: 1, background: '#f8f9fb', borderRadius: 10, padding: 12, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--d-accent)', marginTop: 2 }}>
          ...
          <button
            ...
            style={{
              width: '100%', padding: '12px 0', border: '1.5px solid var(--d-border)',
              borderRadius: 12, background: '#fff', fontSize: 14, fontWeight: 600, color: 'var(--d-ink)',
        ...
        <>
          ...
          {order.notes.length > 0 && (
            <div style={{
              ...
              borderRadius: 10, padding: '10px 12px', marginBottom: 14, fontSize: 13, color: '#92400e',
            }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {!photoAdded ? (
              <button
                ...
                style={{
                  width: '100%', height: 80, border: '2px dashed var(--d-border)', borderRadius: 12,
              ...
              <div
                ...
                style={{
                  width: '100%', height: 80, border: '2px solid #22c55e', borderRadius: 12,
                  background: 'rgba(34,197,94,0.06)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  gap: 8, fontSize: 13, color: '#22c55e', fontWeight: 600, cursor: 'pointer',
                }}
              >
    ...
    if (step === 'en_route_dropoff') {
      return (
        <>
          ...
          {/* Recipient info + Call/Message */}
          <div style={{ background: '#f8f9fb', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: 'var(--d-muted)', marginBottom: 4 }}>RECIPIENT</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--d-ink)' }}>{order.dropoff.name}</div>
              {order.dropoff.phone && (
                <div style={{ fontSize: 12, color: 'var(--d-muted)', marginTop: 2 }}>{order.dropoff.phone}</div>
              )}
            </div>
            <ContactRow
              onCall={handleOpenCall}
          ...
          {order.dropoff.note && (
            <div style={{
              background: '#f0f4ff', border: '1px solid #c7d7fc',
              borderRadius: 10, padding: '10px 12px', marginBottom: 14, fontSize: 13, color: '#3730a3',
            }}>
