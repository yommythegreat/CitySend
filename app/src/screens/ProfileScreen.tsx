  ...
  return (
    <div className="cs-screen cs-enter-right">
      ...
      <div style={{ flex: 1, padding: '0 20px', overflowY: 'auto', scrollbarWidth: 'none' }}>
        ...
        {/* Saved addresses — gated for guests */}
        <div style={{ marginBottom: 16 }}>
          ...
          {isGuest ? (
            <button
              onClick={() => go('auth')}
              style={{
                width: '100%', padding: '16px', background: '#fff', borderRadius: 16,
                border: '1.5px dashed var(--cs-slate-200)', cursor: 'pointer',
                fontFamily: 'var(--cs-font)', textAlign: 'left',
              }}
            >
              <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--cs-accent)' }}>Create an account to save places →</div>
              <div style={{ fontSize: 12, color: 'var(--cs-slate-500)', marginTop: 3 }}>Home, Work, and any frequent stop.</div>
            </button>
          ) : (
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid var(--cs-slate-100)', overflow: 'hidden' }}>
        ...
        {/* Recent deliveries — hidden for guests (no history) */}
        {!isGuest && state.pastDeliveries.length > 0 && (
